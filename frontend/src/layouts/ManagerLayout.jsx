...
const ManagerLayout = () => {
  ...
  const notifications = [
    { id: 1, text: "New leave request from team member", time: "2 min ago", unread: true },
    { id: 2, text: "Worklog submitted for review", time: "1 hour ago", unread: true },
  ];
      ...
      <aside className={`${sidebarCollapsed ? 'w-20' : 'w-64'} ${sidebarBg} ${sidebarText} flex flex-col transition-all duration-300 shadow-lg z-20 border-r ${sidebarBorder}`}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            const activeBg = darkMode ? "bg-blue-600" : "bg-blue-100";
            const activeText = darkMode ? "text-white" : "text-blue-800";
        ...
        <div className={`p-4 border-t ${sidebarBorder}`}>
          {!sidebarCollapsed ? (
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className={`font-medium ${sidebarText}`}>
                    {user?.employee?.fullName || "Manager User"}
                  </p>
                  <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Manager</p>
                </div>
              </div>
              <button 
                onClick={handleLogout}
                className={`w-full flex items-center justify-center gap-2 text-sm ${darkMode ? 'bg-gray-700' : 'bg-gray-100'} ${sidebarText} px-3 py-2 rounded-lg hover:bg-blue-600 dark:hover:bg-blue-100 ${darkMode ? 'hover:text-white' : 'hover:text-blue-800'} transition-all duration-200 border ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}
              >
                <FiLogOut />
                Logout
              </button>
            ...
            <div className="flex flex-col items-center gap-2">
              <div className={`w-8 h-8 rounded-full ${darkMode ? 'bg-blue-600' : 'bg-blue-500'} flex items-center justify-center text-white font-bold text-sm`}>
                {userInitials}
              </div>
              <button 
                onClick={handleLogout}
              ...
              {notificationsOpen && (
                <div className={`absolute right-0 mt-2 w-80 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg border ${sidebarBorder} py-2 z-50`}>
                  ...
                  {notifications.map(notif => (
                    <div key={notif.id} className={`px-4 py-3 ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} transition-colors ${notif.unread ? (darkMode ? 'bg-blue-600/10' : 'bg-blue-50') : ''}`}>
