...
const navigate = useNavigate();
    ...
    const fetchData = async () => {
      ...
      try {
        const [dashRes, chartsRes, jobsRes] = await Promise.all([
          ...
          axios.get('/api/jobs', { headers: { Authorization: `Bearer ${token}` }, params: { status: "Active" } }),
        ]);

        setDashboard(dashRes.data);
        setCharts(chartsRes.data);
        setActiveJobsCount(Array.isArray(jobsRes.data) ? jobsRes.data.length : 0);
      } catch (err) {
  ...
  const stats = [
    ...
    { 
      ...
      change: dashboard && dashboard.totalEmployees ? `${((dashboard.presentToday / dashboard.totalEmployees) * 100).toFixed(1)}%` : '—', 
      trend: "up", 
    ...
    { 
      ...
      change: charts?.departments ? `Total: ${charts.departments.length}` : '+1', 
      trend: "up", 
      color: darkMode ? "bg-purple-500/20 text-purple-400" : "bg-purple-100 text-purple-700" 
    },
    { 
  ...
  const attendanceTrendData = charts?.attendance?.map((r) => ({ day: monthNames[(Number(r.month)-1)%12] || 'N', present: Number(r.count), late: 0 })) || [
  ...
  const employeeDistributionData = charts?.departments?.map((d, i) => ({ name: d.department || `Dept ${i+1}`, value: d.count || 0, color: [colors.primary, colors.secondary, colors.warning, colors.tertiary, '#EC4899'][i % 5] })) || [
  ...
  const totalLeavesThisYear = charts?.leaves ? charts.leaves.reduce((s, r) => s + Number(r.count), 0) : null;
  const leaveData = charts?.leaves ? [
    { type: "Approved (year)", count: totalLeavesThisYear || 0, color: colors.primary },
    { type: "Pending", count: dashboard ? dashboard.pendingLeaves : 0, color: "#10B981" },
    { type: "On Leave Today", count: dashboard ? dashboard.onLeaveToday : 0, color: "#F59E0B" },
  ] : [
  ...
  const attendanceRate = dashboard && dashboard.totalEmployees
